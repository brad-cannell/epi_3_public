...
* Please make sure the output is set to `html_notebook` -- not `html_document`.
* Please name this file `lab_module_name.Rmd`.
...
* Data set/frame names, variable names, and other key words should be written wit **bold** text (e.g., "Please download **chocolate study 1.xls**".
...
**Libby,** So far (2022-10-18), there aren't any questions on the lab require R coding. If that remains the case, then we don't need to write anything in this document. However, if we do end up adding some questions to the lab that require R coding, then let's please follow the layout below. 

**NOTE:** I don't think we need to (or should) have every task/question written in the R code file. Just the questions that relate to R coding. This is a little different that what we did in the Intro to R course, so we may need to play around with it and see how it goes. 
