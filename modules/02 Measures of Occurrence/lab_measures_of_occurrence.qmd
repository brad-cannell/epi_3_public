...
```{r}
plot_lab_measures_of_occurrence <- ggplot(pop, aes(month, "p", fill = status_f)) + 
  geom_raster(hjust = 1) +
...
person_months_at_risk <- 1 + 4 + 7 + 5 + 25 + 25 + 13 + 25 + 24 + 2 + 15 + 12 + 2
...
pop |> 
  # Count the number of rows with each value of status_f.
  # Each row is a person-month, so the total number of rows with each value 
  # is also the total person-months with each value.
  count(status_f)
```

3. We can even have R calculate the person-months at risk by person, as shown below.

```{r}
# Have R calculate the total person-months at risk by person
pop |>
  # Use group_by() to repeat the calculation below for each person
  group_by(person_f) |> 
  # Count the number of rows with each value of status_f.
  # Each row is a person-month, so the total number of rows with each value 
  # is also the total person-months with each value.
  count(status_f) |> 
  # Keep only the "At Risk" result to make the results easier to read.
  filter(status_f == "At Risk")
```
...
* The total person-months at risk in the simulated population during the 24 months of follow-up is 147. Each person contributed the following amounts of person-months at risk:
- Person 01: 0
- Person 02: 0
- Person 03: 1
- Person 04: 4
