...
```{r}
person_time_plot <- function(.data, 
                             .x, 
                             .y,
                             .status, 
                             .censored = "No",
  ...
  # Set colors depending on if there is a censored status value or not
  if (.censored == "No") {
    colors <- c("gray", "orange", "red", "black")
  } else if (.censored == "Yes") {
    colors <- c("gray", "orange", "red", "black", "yellow")
  } else {
    stop("The value of .censored must be 'No' or 'Yes'.")
  }
  
  # Calculate the number of time periods in the data
  # This is used to label the x-axis in scale_x_continuous below
  time_periods <- pull(.data, {{ .x }}) |> unique()
  # Keep only the values higher than zero
  # This is used in geom_vline to create the light gray vertical line 
  # at each time period after baseline
  time_periods_positive <- time_periods[time_periods > 0]
  # Calculate the number of unique people in the data
  # This is used for facet_wrap below
  n_facets <- pull(.data, {{ .y }}) |> unique() |> length()
  
  ggplot(.data, aes({{ .x }}, "person", fill = {{ .status }})) + 
    geom_raster(hjust = 1) +
    # Create a bar marking the start of follow up
    geom_vline(xintercept = 0, color = "blue") +
    # Add vertical lines to mark the start of each month
    geom_vline(xintercept = time_periods_positive, color = "#f9f9f9") +
    # Show all months
    scale_x_continuous(.x_lab, breaks = time_periods) +
    facet_wrap(vars({{ .y }}), nrow = n_facets, strip.position = "left") +
    # Change fill colors
    scale_fill_manual(.status_lab, values = colors) +
    # Change x-axis label
    ylab(.y_lab) +
    theme(
      # Remove gray background
...
```{r}
# Rearrange data for geom_segment
test_pop_segment <- tribble(
  ~person_f, ~status_f,       ~x, ~xend, ~y, ~yend,
  "01",      "Diseased",      -1, 4,     1,  1,
  "02",      "Pre-follow-up", -1, 0,     2,  2,
  "02",      "At Risk",        0, 4,     2,  2,
  "03",      "Censored",      -1, 1,     3,  3,
  "03",      "At Risk",        1, 2,     3,  3,
  "03",      "Diseased",       2, 4,     3,  3,
  "04",      "Pre-follow-up", -1, 0,     4,  4,
  "04",      "At Risk",        0, 2,     4,  4,
  "04",      "Censored",       2, 4,     4,  4,
) |> 
  mutate(
    person_f = factor(person_f),
...
```{r}
ggplot(test_pop_segment) +
  geom_segment(aes(x = x, xend = xend, y = 1, yend = 1, color = status_f), size = 3) +
...
There are valid philosophical debates to have regarding whether “truth” is an absolute thing or a moving target, whether it is knowable or unknowable, and whether it exists at all. 
...
🔴 Come back and improve explanations
...
🔴 Create the graphic on slide 5?

🔴 Create the graphic on slide 6?
