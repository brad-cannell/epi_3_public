...
**Note**: This file is getting long. You may want to break it up into multiple different files. One for each different type of graphic.
...
people <- 1:10
months <- -1:12
pt_df <- tidyr::expand_grid(person = people, month = months)
...
plot_ic_cause_murder_dag <- ggdag(ic_cause_murder_dag, text = FALSE, use_labels = "label") +
  theme_dag()
```
...
plot_ic_no_cause_murder_dag <- ggdag(ic_no_cause_murder_dag, text = FALSE, use_labels = "label") +
  theme_dag()
```
...
Issues:
- When combining side-by-side (the default), the labels still cover the arrows.
- When stacking, some of the arrows are still covered by labels and some of the nodes are partially cut off the plot. 
- So manually set label locations
...
```{r eval=FALSE}
ggsave("/Users/bradcannell/Desktop/ic_murder_plots.png", width = 13.3, height = 7.5)
```
...
Next, we want to add segments over the top of the current edges and color them red. Howver, it's really hard to guess the correct coordinates for the segments. For example...
