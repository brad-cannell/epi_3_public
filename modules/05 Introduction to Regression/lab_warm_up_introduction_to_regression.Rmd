...
2. x_cont: The change in the mean value of `y_cont` when `x_cont` changes from zero to one. 
...
2. x_cont: The average change in the natural log of the odds of `y_cont` when `x_cont` changes from zero to one.
...
2. x_cont: The average change in the natural log of the mean of `y_count` when `x_cont` changes from zero to one.
...
1. Intercept: The mean value of deep abdominal adipose tissue when waist circumference is -9.580. 
...
### Categorical regressor (tall)

```{r}
daat_waist <- daat_waist %>% 
  mutate(
    tall  = if_else(waist < mean(waist), 0, 1),
    tall_f = factor(tall, 0:1, c("No", "Yes"))
  )
daat_waist
```

```{r}
daat_by_tall_f <- daat_waist %>% 
  group_by(tall_f) %>% 
  mean_table(daat)
...
```{r}
ggplot(daat_waist, aes(x = tall_f, y = daat)) +
  geom_point() +
  geom_segment(
    aes(x = c(0.75, 1.75), y = mean, xend = c(1.25, 2.25), yend = mean), 
    size = 1.5, color = "red", data = daat_by_tall_f
  )
```
...
2. waist: The average change in deep abdominal adipose tissue for tall people compared to not tall people is 8.854.
...
```{r}
em_age_70 <- -3.180 + (70 * 0.02127)
em_age_70
```

```{r}
em_age_71 <- -3.180 + (71 * 0.02127)
em_age_71
```
...
3. Participants reported 0.8 times the number of drinks at their last drinking episode for each one-year increase in age.

```{r}
drinks_age_50 <- 0.113739 + (50 * -0.001131)
drinks_age_50
```

```{r}
drinks_age_51 <- 0.113739 + (51 * -0.001131)
drinks_age_51
```
